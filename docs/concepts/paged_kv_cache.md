...
This project implements the paged KV cache to understand the memory management principles. New: The throughput overhead (~1.4x slower than standard at bs=4, ~1.8x at bs=256) is the remaining cost of Python-level scatter/gather after vectorization (batched `index_select` reads, advanced-indexing writes). Production systems (vLLM) close this gap entirely with custom CUDA kernels (`paged_attention_v1/v2`).
